...
- [x] CSS가 `[hidden]` 상태를 덮어써 카메라 오버레이가 남는 문제를 수정한다.
- [x] 스트림 확보 즉시 카메라 오버레이를 숨겨 영상 시작 대기가 화면을 막지 않는다.
- [x] 스캔 상태를 카메라 패널 배지와 시작 버튼 텍스트에 명확히 표시한다.
- [x] 1개/2x1 스캔 모드를 선택할 수 있고, 2x1에서는 카메라 프레임 좌우를 각각 디코딩한다.
- [x] 1개/2x1 스캔 모드 UI를 스캔 방식 segmented control로 정리한다.
...
- 기본 스캔 모드는 기존처럼 QR 1개 전용으로 두고, 2x1 송출을 받을 때만 사용자가 2x1 스캔 모드를 선택한다.
